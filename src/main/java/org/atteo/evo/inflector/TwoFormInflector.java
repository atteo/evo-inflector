    ...
    protected void irregular(String singular, String plural) {
        if (singular.charAt(0) == plural.charAt(0)) {
            rules.add(new RegExpRule(
                    "(?i)(" + singular.charAt(0) + ")" + singular.substring(1) + "$", "$1" + plural.substring(1)));
        } else {
            rules.add(new RegExpRule(
                    toUpperCase(singular.charAt(0)) + "(?i)" + singular.substring(1) + "$",
                    toUpperCase(plural.charAt(0)) + plural.substring(1)));
            rules.add(new RegExpRule(
                    toLowerCase(singular.charAt(0)) + "(?i)" + singular.substring(1) + "$",
